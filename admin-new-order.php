		...
		<?php
			if ( $totals = $order->get_order_item_totals() ) {
				...
				foreach ( $totals as $total ) {
					...
					</tr><?php if ($total['value'] === 'Delivery to Student') {?>
                    <tr>
                        <th class="td" scope="row" colspan="2" >Student Name</th>
                        <td class="td" style="text-align:left;"><?php echo  get_post_meta( $order->id, 'Student Name', true );?></td>
                    </tr>
                    <?php }else if ($total['value'] === 'Collection') {?>
					<tr>
                        <th class="td" scope="row" colspan="2">Collected by:</th>
                        <td class="td" style="text-align:left;"><?php echo  get_post_meta( $order->id, 'PickUp Name', true );?></td>
                    </tr>
                    <?php }?><?php
				}
			}
